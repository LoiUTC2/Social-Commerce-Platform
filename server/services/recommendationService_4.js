...
const cacheManager = require('./optimizedCacheStrategy');
...
async function prepareUserEntityMatrix() {
    ...
    const users = [];
    const userMap = new Map(); // Map để lưu userId/sessionId và loại (user/session)
    interactions.forEach(i => {
        if (i.author?._id) {
            const userId = i.author._id.toString();
            if (!userMap.has(userId)) {
                userMap.set(userId, { id: userId, type: 'user' });
                users.push(userId);
            }
        } else if (i.sessionId) {
            const sessionId = i.sessionId;
            if (!userMap.has(sessionId)) {
                userMap.set(sessionId, { id: sessionId, type: 'session' });
...
async function trainUserShopModel() {
    ...
    try {
        ...
        try {
            ...
            await redisClient.setex('user_shop_model_data', 600, JSON.stringify(redisModelData));
...
async function loadUserShopModel() {
    ...
    try {
        ...
        try {
            ...
            await redisClient.setex(cacheKey, 600, JSON.stringify(redisModelData));
...
async function getUserShopRecommendations(userId, sessionId, limit = 10, entityType = 'all', role = 'user', options = {}) {
    ...
    const {
        enableCache = true,
        cacheTimeout = 600, //3phuts  // 30 phút, 1800
...
// Lấy thông tin chi tiết của các thực thể được gợi ý từ DB (User hoặc Shop)
...
async function trainMatrixFactorization() {
    ...
    try {
        ...
        // Cache vào Redis
        try {
            await redisClient.setex('mf_model', 600, JSON.stringify(modelData));
...
async function loadMatrixFactorizationModel() {
    ...
    try {
        ...
        // Cache vào Redis
        try {
            await redisClient.setex(cacheKey, 600, JSON.stringify(modelData));
            console.log('💾 Đã cache MF model vào Redis');
        } catch (redisCacheError) {
...
// ✨ ENHANCED: Chuẩn bị dữ liệu TF-IDF với thông tin Flash Sale tốt hơn
async function prepareTfIdfMatrix() {
    // Sử dụng cache manager để optimize
    return await cacheManager.getCachedRecommendations(
        cacheManager.cacheKeys.tfidf,
        async () => {
            console.log('🔄 Preparing fresh TF-IDF matrix...');

            // Lấy products với thông tin flash sale đã được populate tự động
            const products = await Product.find({ isActive: true })
                .select('name description hashtags currentFlashSale price mainCategory stock')
                .populate('currentFlashSale.flashSaleId', 'name description hashtags')
                .lean();
            ...
            // Lấy Flash Sales với thông tin sản phẩm
            ...
            const itemIds = [];
            const itemTypes = [];
            const itemMetadata = []; // ✨ THÊM: Metadata để AI quyết định tốt hơn

            // Thêm nội dung sản phẩm với thông tin flash sale
            products.forEach(product => {
                let content = `${product.name} ${product.description} ${product.hashtags.join(' ')}`;

                // ✨ ENHANCED: Thêm context flash sale vào TF-IDF
                if (product.currentFlashSale?.isActive && product.currentFlashSale.flashSaleId) {
                    content += ` ${product.currentFlashSale.flashSaleId.name} ${product.currentFlashSale.flashSaleId.description || ''}`;
                }
                ...
                itemMetadata.push({
                    hasFlashSale: !!product.currentFlashSale?.isActive,
                    flashSaleId: product.currentFlashSale?.flashSaleId?._id,
                    flashSaleEndTime: product.currentFlashSale?.endTime,
                    flashSalePrice: product.currentFlashSale?.salePrice,
                    originalPrice: product.price,
                    mainCategory: product.mainCategory,
                    stock: product.stock,
                    stockLimit: product.currentFlashSale?.stockLimit,
                    soldInFlashSale: product.currentFlashSale?.soldCount || 0
                });
            });
            ...
            posts.forEach(post => {
                ...
                itemTypes.push('post');
                itemMetadata.push({ hasFlashSale: false });
            });
            ...
            flashSales.forEach(flashSale => {
                ...
                itemMetadata.push({
                    ...
                    productsCount: flashSale.products?.length || 0
                });
            });
        ...
        cacheManager.cacheDurations.tfidf
    );
}
...
// ✨ ENHANCED: Gợi ý với ưu tiên Flash Sale thông minh
async function getContentBasedRecommendations(itemId, itemType = 'product', limit = 10, options = {}) {
    try {
        const { prioritizeFlashSale = true, includeExpiringSoon = true } = options;
        ...
        if (!cached) {
            ...
            tfidfData = {
                documents: result.tfidf.documents,
        ...
        const { documents, itemIds, itemTypes, itemMetadata } = tfidfData;
        ...
        const similarities = documents.map((doc, idx) => {
            ...
            const similarity = cosineSimilarity(documents[itemIdx], doc);
            const metadata = itemMetadata[idx];

            // ✨ ENHANCED: Tính toán điểm số với ưu tiên Flash Sale
            let score = similarity;

            if (prioritizeFlashSale && metadata.hasFlashSale) {
                score *= 1.3; // Boost 30% cho items có flash sale
            }

            if (includeExpiringSoon && metadata.endTime) {
                const timeLeft = new Date(metadata.endTime) - now;
                const hoursLeft = timeLeft / (1000 * 60 * 60);

                // Boost items sắp hết flash sale (trong 24h)
        ...
        }).filter(s => s && s.similarity > 0);

        // Sắp xếp theo score (đã tính ưu tiên)
        const topItems = similarities
            .filter(s => ['product', 'flashsale'].includes(s.itemType))
            .sort((a, b) => b.score - a.score)
            .slice(0, limit);

        // Lấy thông tin chi tiết với virtual fields
        const result = [];
        for (const { itemId, itemType, similarity, score, metadata } of topItems) {
            try {
                if (itemType === 'product') {
                    const product = await Product.findById(itemId)
                        .select('name description price hashtags images currentFlashSale stock soldCount mainCategory')
                        .populate('currentFlashSale.flashSaleId', 'name description startTime endTime')
                        .lean();

                    if (product) {
                        // ✨ Sử dụng virtual fields để tính giá
                        ...
                        result.push({
                            ...
                            pricing: {
                                ...
                                isInFlashSale: productDoc.isInFlashSale,
                                discountInfo: productDoc.discountInfo,
                                discountPercentage: productDoc.discountPercentage
                            },
                            flashSaleInfo: product.currentFlashSale,
                            availability: {
                                inStock: product.stock > 0,
                                totalStock: product.stock,
                                flashSaleStock: product.currentFlashSale?.stockLimit,
                                flashSaleSold: product.currentFlashSale?.soldCount || 0,
                                flashSaleRemaining: product.currentFlashSale?.isActive ?
                                    (product.currentFlashSale.stockLimit - (product.currentFlashSale.soldCount || 0)) : null
                            }
                        });
                    }
                    ...
                    const flashSale = await FlashSale.findById(itemId)
                        ...
                        .populate({
                            path: 'products.product',
                            select: 'name images mainCategory price hashtags'
                        })
                        .lean();
...
// Hàm lấy content-based recommendations từ lịch sử user (ENHANCED với Flash Sale)
async function getContentBasedRecommendationsFromUserHistory(userId, sessionId, limit = 20) {
    try {
        // ✨ Sử dụng cache manager cho user history recs
        const cacheKey = `recs:content:${userId || sessionId}:${limit}:v2`;

        return await cacheManager.getCachedRecommendations(
            cacheKey,
            async () => {
                console.log('🔄 Generating fresh content-based recommendations...');
                ...
                const recentInteractions = await UserInteraction.find({
                    ...
                    .sort({ timestamp: -1 })
                    .limit(15) // Tăng limit để có nhiều data hơn
                    .lean();
                ...
                const contentRecs = new Map(); // Sử dụng Map để tránh duplicate và lưu score
                const processedItems = new Set(); // Tránh xử lý trùng lặp

                // Xử lý từng interaction với trọng số
                for (const interaction of recentInteractions) {
                    try {
                        // Tính trọng số dựa trên action type và thời gian
                        let actionWeight = 1;
                        switch (interaction.action) {
                            case 'purchase': actionWeight = 3; break;
                            case 'add_to_cart': actionWeight = 2.5; break;
                            case 'like': actionWeight = 2; break;
                            case 'save': actionWeight = 2; break;
                            case 'view': actionWeight = 1; break;
                            case 'search': actionWeight = 1.5; break;
                        }

                        // Time decay (càng gần đây càng có trọng số cao)
                        const daysSince = (Date.now() - new Date(interaction.timestamp)) / (24 * 60 * 60 * 1000);
                        const timeWeight = Math.max(0.1, 1 - daysSince / 7); // Giảm dần trong 7 ngày
                        ...
                        if (interaction.targetType === 'search' && interaction.searchSignature) {
                            ...
                            searchRecs.forEach(rec => {
                                const key = `${rec._id}:product`;
                                if (!processedItems.has(key)) {
                                    const score = actionWeight * timeWeight * 0.8; // Search có trọng số thấp hơn
                                    contentRecs.set(key, { item: rec, score, source: 'search' });
                                    processedItems.add(key);
                                }
                            });

                        } else if (interaction.targetId && /^[0-9a-fA-F]{24}$/.test(interaction.targetId.toString())) {
                            // ✨ ENHANCED: Gợi ý dựa trên Flash Sale hoặc sản phẩm với options
                            ...
                            const itemRecs = await getContentBasedRecommendations(
                                ...
                                Math.ceil(limit / 3),
                                {
                                    prioritizeFlashSale: true,
                                    includeExpiringSoon: true,
                                    boostSimilarCategory: true
                                }
                            );

                            itemRecs.forEach(rec => {
                                const key = `${rec._id}:${rec.type}`;
                                if (!processedItems.has(key)) {
                                    let score = actionWeight * timeWeight;

                                    // ✨ Boost Flash Sale items
                                    if (rec.type === 'flashsale' || rec.pricing?.isInFlashSale) {
                                        score *= 1.4;
                                    }

                                    // ✨ Boost items sắp hết sale
                                    if (rec.pricing?.discountInfo?.hoursLeft && rec.pricing.discountInfo.hoursLeft <= 24) {
                                        score *= 1.3;
                                    }
                ...
                // Sắp xếp theo điểm số và lấy top items
                const sortedRecs = Array.from(contentRecs.values())
                    .sort((a, b) => b.score - a.score)
                    .slice(0, limit);

                // ✨ Fetch thông tin chi tiết với virtual fields
                const result = [];
                for (const { item, score, source } of sortedRecs) {
                    try {
                        const [itemId, itemType] = item._id ? [item._id.toString(), item.type] : [item.itemId, item.itemType];

                        if (itemType === 'product') {
                            const product = await Product.findById(itemId)
                                .populate('currentFlashSale.flashSaleId', 'name description')
                                .lean();

                            if (product) {
                                // ✨ Sử dụng virtual fields để tính giá chính xác
                                const productDoc = new Product(product);
            ...
            cacheManager.cacheDurations.flashSaleRecs // 3 phút vì có Flash Sale data
        );
    ...
    try {
        ...
        if ((!collaborativeRecs || collaborativeRecs.length === 0) &&
            ...
            if (fallbackResult.length > 0) {
                await redisClient.setex(cacheKey, 300, JSON.stringify(fallbackResult));
            }
        ...
        if (result.length > 0) {
            try {
                await redisClient.setex(cacheKey, 300, JSON.stringify(result)); // Cache 30 phút
            } catch (cacheError) {
                console.warn('⚠️ Không thể cache result:', cacheError.message);
            }
        }
...
// ✨ ENHANCED: Flash Sale recommendations với cache strategy
async function getFlashSaleRecommendations(userId, sessionId, limit = 10, role = 'user') {
    try {
        const cacheKey = cacheManager.cacheKeys.flashSaleRecs(userId, sessionId, limit);

        return await cacheManager.getCachedRecommendations(
            cacheKey,
            async () => {
                console.log('🔄 Generating fresh Flash Sale recommendations...');

                // 1. Lấy Flash Sales đang active từ cache nếu có
                let activeFlashSales;
                try {
                    const cachedFlashSales = await redisClient.get(cacheManager.cacheKeys.activeFlashSales);
                    if (cachedFlashSales) {
                        activeFlashSales = JSON.parse(cachedFlashSales);
                    } else {
                        activeFlashSales = await FlashSale.find({
                            isActive: true,
                            ...
                            .populate({
                                path: 'products.product',
                                select: 'name mainCategory price hashtags currentFlashSale images stock soldCount description',
                                populate: {
                        ...
                        await redisClient.setex(
                            cacheManager.cacheKeys.activeFlashSales,
                            cacheManager.cacheDurations.activeFlashSales,
                            JSON.stringify(activeFlashSales)
                        );
                    }
                } catch (cacheError) {
                    console.warn('⚠️ Lỗi cache Flash Sales:', cacheError.message);
                    activeFlashSales = [];
                }
                ...
                // 3. Tính điểm cho từng Flash Sale
                const scoredFlashSales = activeFlashSales.map(flashSale => {
                    let score = 0;

                    // Base score từ thống kê
                    score += (flashSale.stats?.totalViews || 0) * 0.1;
                    score += (flashSale.stats?.totalPurchases || 0) * 2;

                    // Tính thời gian remaining
                    const timeLeft = new Date(flashSale.endTime) - new Date();
                    const hoursLeft = timeLeft / (1000 * 60 * 60);

                    // Boost cho flash sale sắp kết thúc
                    if (hoursLeft <= 24) score *= 1.5;
                    if (hoursLeft <= 6) score *= 2;

                    // User preference matching
                    userInteractions.forEach(interaction => {
                        if (interaction.targetType === 'product' && flashSale.products) {
                            const hasRelatedProduct = flashSale.products.some(p =>
                                p.product?._id?.toString() === interaction.targetId ||
                                p.product?.mainCategory?.toString() === interaction.targetDetails?.mainCategory
                            );
                            if (hasRelatedProduct) score += 5;
                        }
                    });

                    return { ...flashSale, aiScore: score, hoursLeft };
                });

                // 4. Lấy products từ Flash Sales với pricing chính xác
                const enhancedProducts = [];
                for (const flashSale of scoredFlashSales) {
                    if (flashSale.products) {
                        for (const item of flashSale.products) {
                            if (item.product) {
                                // ✨ Sử dụng virtual fields để có giá chính xác
                                ...
                                enhancedProducts.push({
                                    ...
                                    hoursLeft: flashSale.hoursLeft,
                                    aiScore: flashSale.aiScore,
                                    // ✨ Pricing từ virtual fields
                                    pricing: {
                ...
                console.log(`✅ AI Flash Sale Recs: ${result.flashSales.length} flash sales, ${result.products.length} products`);
                return result;
            },
            cacheManager.cacheDurations.flashSaleRecs
        );
...
// ✨ Hàm để invalidate cache khi có thay đổi
async function invalidateRecommendationCache(type, targetId) {
    try {
        switch (type) {
            case 'flashsale':
                await cacheManager.invalidateFlashSaleCache(targetId);
                break;
            case 'product':
                // Invalidate related caches
                const keys = await redisClient.keys('recs:*');
                if (keys.length > 0) {
