...
async function trainUserShopModel() {
    ...
    try {
        ...
        const modelData = {
            userEmbedding: userEmbeddingArray, // Lưu dưới dạng array
            entityEmbedding: entityEmbeddingArray, // Lưu dưới dạng array
        ...
        // Tạo model object để return và cache
        const model = {
        ...
        // Cache vào Redis với key riêng
        try {
            await redisClient.setex('user_shop_model', 1800, JSON.stringify(model));
            console.log('💾 Đã cache User-Shop model vào Redis');
        } catch (redisError) {
...
async function loadUserShopModel() {
    ...
    //Nếu hết hạn hoặc chưa có, lấy từ redis
    const cacheKey = 'user_shop_model'; // Redis key riêng cho User-Shop model

    try {
        const cached = await redisClient.get(cacheKey);
        if (cached) {
            console.log('📦 Tìm thấy User-Shop model trong Redis cache');
            modelCache = JSON.parse(cached);
            modelCacheTime = Date.now();
            return modelCache;
        }
    } catch (redisError) {
    ...
    // Nếu trên redis cũng không có, lấy FILE RIÊNG CHO USER-SHOP MODEL
    ...
    try {
        ...
        // Kiểm tra embedding data - ĐÂY LÀ VẤN ĐỀ CHÍNH
        if (!modelData.userEmbedding || !modelData.entityEmbedding) {
            console.error('❌ Embedding data không tồn tại');
            return await trainUserShopModel();
        }

        // Chuyển đổi từ object về array nếu cần (TensorFlow.js thường lưu dưới dạng object)
        ...
        // Cache vào Redis
        await redisClient.setex(cacheKey, 3600, JSON.stringify(modelCache));
        console.log('✅ User-Shop Model loaded from file và cached');
