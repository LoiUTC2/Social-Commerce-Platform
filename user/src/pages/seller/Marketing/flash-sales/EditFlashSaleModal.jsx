...
import { Plus, X, Search, Package, DollarSign, Hash, Calendar, Edit, AlertCircle, CheckCircle } from "lucide-react"
import { toast } from "sonner"
import { updateFlashSale } from "../../../../services/flashSaleService"
import { getProductsByShopForSeller } from "../../../../services/productService"
import { useAuth } from "../../../../contexts/AuthContext"

export default function EditFlashSaleModal({ isOpen, onClose, flashSale, onSuccess }) {
    const { user } = useAuth()
    ...
    // Initialize form when flashSale changes
    useEffect(() => {
        if (isOpen && flashSale) {
            fetchProducts()
            initializeForm()
        ...
        if (isSelected) {
            ...
            setSelectedProducts((prev) => [
                ...prev,
                {
                    ...
                    stockLimit: Math.min(product.stock, 100),
                    soldCount: 0,
                },
            ])
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await updateFlashSale(flashSale._id, submitData)

            if (response.success) {
                toast.success("Cập nhật Flash Sale thành công!")
                onSuccess()
            } else {
                throw new Error(response.message || "Cập nhật Flash Sale thất bại")
            }
        } catch (error) {
            toast.error("Lỗi", { description: error.message || "Cập nhật Flash Sale thất bại" })
        } finally {
            setLoading(false)
        }
    }
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="text-sm text-gray-500">
                                                    ...
                                                    {selectedProduct.soldCount > 0 && (
                                                        <span className="ml-2 text-green-600">• Đã bán: {selectedProduct.soldCount}</span>
                                                    )}
                                                </div>
                                            </div>
                                            <Button
                                                ...
                                                <div className="text-sm text-gray-500">
                                                    ...
                                                    {selectedProduct.soldCount > 0 && (
                                                        <span className="block text-green-600">
                                                            Còn lại: {selectedProduct.stockLimit - selectedProduct.soldCount}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
