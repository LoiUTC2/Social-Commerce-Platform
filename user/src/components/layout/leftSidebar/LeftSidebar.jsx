...
const LeftSidebar = ({ className }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setVisibleComponents(new Set([0, 1])) // Hiển thị tất cả components hiện có
    }, 300)
  ...
  return (
    <div className={cn("relative h-full", className)}>
      ...
      >
        <div style={{ direction: "ltr" }} className="space-y-6 pb-4">
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error("LeftSidebar Component Error:", error, errorInfo)
    this.props.onError?.()
  }
